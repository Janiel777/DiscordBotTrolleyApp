...
from INSOAPIQuery.utils.models import DeveloperMetrics, MilestoneData
from INSOAPIQuery.utils.queryRunner import run_graphql_query

local_tz = timezone('America/Puerto_Rico')  # Ajusta según la zona horaria que estés usando
...
def outliersRemovedAverage(scores: Iterable) -> float:
    ...
    total = sum(scores, 0) - largestVal - smallest_elem
...
def getCurrentSprintIndex(date: datetime, cutoffs: list[datetime]):
    ...
    for cutoff in cutoffs:
        cutoff = local_tz.localize(cutoff)
    ...
    while hasAnotherPage:
        ...
        for issue in issues:
            ...
            if issue["content"]["milestone"]["title"] != milestone:
                continue
            if issue["content"].get("closed", False) or (
                    shouldCountOpenIssues and not issue["content"].get("closed", False)):
                # Si shouldCountOpenIssues es True, tratamos este issue como cerrado y lo atribuimos a un manager
                if shouldCountOpenIssues and not issue["content"].get("closed", False):
                    closedBy = managers[0]  # Simulamos que el primer manager cerró el issue
                    issue["content"]["closed"] = True  # Cambiamos el estado del issue a cerrado
                    logger.info(
                        f"[Issue #{issue['content'].get('number')}]({issue['content'].get('url')}) "
                        f"was simulated to be closed by manager {closedBy}."
                    )
                else:
                    # Si el issue está cerrado, verificamos quién lo cerró
                    closedByList = issue["content"]["timelineItems"]["nodes"]
                    closedBy = closedByList[-1]["actor"]["login"]

                # Si el issue está cerrado por alguien que no es manager y shouldCountOpenIssues es False, lo omitimos
                if not shouldCountOpenIssues and closedBy not in managers:
                    logger.warning(
                        f"[Issue #{issue['content'].get('number')}]({issue['content'].get('url')}) "
                        f"was closed by non-manager {closedBy}. Only issues closed by managers are accredited. "
                        f"Managers for this project are: {managers}"
                    )
                    continue  # Omitimos el issue si no fue cerrado por un manager
            ...
            print(issue)
            # En lugar de usar datetime.fromisoformat
            createdAt = parser.isoparse(issue["content"]["createdAt"]).replace(tzinfo=None)
            issueScore = (
            ...
            for dev in issue["content"]["assignees"]["nodes"]:
                ...
                if issue["content"]["closedAt"] is not None:
                    closedAt = parser.isoparse(issue["content"]["closedAt"]).replace(tzinfo=None) if issue["content"]["closedAt"] else None
